...
flist = grep(pattern = "*grow_stratum.yearly",list.files("./"),value = T)
...
for(i in 1:n){
  ...
  datlist[[i]] = load_rhessys(file0, Load_id = id) %>%
    agg_rhessys(., Grouplist = c("syr","stratumID"), Agglist = vlist, Method = "mean", Scale = "yearly")
...
vlist2 = c("AGBc", "height", "LAI", "NPP","GPP","NPP.GPP")
...
dat1 = dat1 %>% mutate(
  ...
  dplyr::select(all_of(c("tag1","tag2","syr","stratumID",vlist2))) %>%
  dplyr::filter(stratumID == 79708)

dat1$tag1 = dat1$tag1 %>% factor()
#dat1$tag2 = dat1$tag2 %>% factor(., levels = tag2list %>% unique())
dat1$tag2 = dat1$tag2 %>% factor(.)

plot_single(dat1, Varlist = vlist2, Facet = c("var","tag1"), Color = "tag2", Scales = "free_y",Ncol = 2,
            Legend = "right") +
  labs(
    x = "Simulation Year",
    y = NULL,
